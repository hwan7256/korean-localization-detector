...
def list_services(limit: int = Query(50, ge=1, le=100), offset: int = Query(0), source: str = Query(None), min_score: int = Query(0)):
    db = get_db()
    conds, params = ["1=1"], []
    ...
    q = f"SELECT s.*, a.localization_score, a.summary_ko, a.localization_reason, a.free_tier, a.created_at as analyzed_at FROM discovered_services s LEFT JOIN analysis_reports a ON s.id = a.service_id WHERE {where} ORDER BY a.localization_score DESC NULLS LAST LIMIT ? OFFSET ?"
